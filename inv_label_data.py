#!/usr/bin/env python3
"""
Inverse auto-labeling script using a Qwen OCR model with confidence gating.
...
- threshold: float in [0,1]; if prediction confidence < threshold, use ground truth, else use prediction
...
def auto_label(
	...
	if output_parquet is None:
		threshold_str = "{:.3f}".format(threshold).rstrip("0").rstrip(".")
		output_parquet = input_parquet.with_name(f"inv-train-{threshold_str}.parquet")
	else:
	...
	for i, ((pred, conf), gt, identifier, image) in enumerate(zip(all_results, all_gt, all_ids, all_images)):
		chosen = gt if conf < threshold else pred if pred.strip() else gt
		chosen_texts.append(chosen)
		preds.append(pred)
		confs.append(float(conf))
		chosen_src.append("GT" if conf < threshold else "PRED" if pred.strip() else "GT")
...
def parse_args() -> argparse.Namespace:
	...
	p.add_argument("--threshold", type=float, required=True, help="Confidence threshold in [0,1]; predictions below this threshold are replaced with GT")
